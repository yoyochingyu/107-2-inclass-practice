...
```{r ans11}
#typeof(finStatement)
logic1 <- finStatement[[16]] < 0
#sum(logic1)
loss1Companies <- finStatement[[5]][logic1]
#length(loss1Companies)
loss1Companies
...
```{r ans12}
logic2 <- finStatement[[33]]<0
#sum(logic2)
loss2Companies <- finStatement[[5]][logic2]
#length(loss2Companies)
...
```{r ans13}
logic4 <- finStatement[[16]] > 0  & logic2
logic5 <- logic1 & finStatement[[33]]>0
logic6 <- logic4 | logic5
only1LossCompanies <- finStatement[[5]][logic6]
only1LossCompanies
length(only1LossCompanies)
# only1LossCompanies
```
...
subsample <- finStatement[logic6==TRUE,]
#length(subsample$公司名稱)
#class(subsample)
subsample
# subsample
```
...
#names(exhibitionList[[1]])[[5]]
showInfo1 <- exhibitionList[[1]]$showInfo
#showInfo1
#exhibitionList[[1]]
#list1 <- exhibitionList[[1]]$title
#list2 <- showInfo1[c(1,2,3,5,8)]
#list3 <- exhibitionList[[1]]$sourceWebName
#showInfo1[[1]]
listforall <- list(exhibitionList[[1]]$title,showInfo1[[1]][[1]],showInfo1[[1]][[2]],showInfo1[[1]][[3]],showInfo1[[1]][[5]],showInfo1[[1]][[8]],exhibitionList[[1]]$sourceWebName)
names(listforall) <- c("title","time","location","locationName","price","endTime","sourceWebName")
listforall
df_oneShow  <- as.data.frame(listforall,stringsAsFactors = FALSE)
#class(df_oneShow$title)
df_oneShow
# df_oneShow
```
...
showInfo2 <- exhibitionInput$showInfo
listforall <- list(exhibitionInput$title,showInfo2[[1]][[1]],showInfo2[[1]][[2]],showInfo2[[1]][[3]],showInfo2[[1]][[5]],showInfo2[[1]][[8]],exhibitionInput$sourceWebName)
names(listforall) <- c("title","time","location","locationName","price","endTime","sourceWebName")
#listforall
df_oneShow  <- as.data.frame(listforall,stringsAsFactors = FALSE)
#class(df_oneShow$title)
