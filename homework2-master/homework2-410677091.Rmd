...
```{r ans12}
varia <- c("一","二","三","四")
names(studentRecord[[4]])[c(1,2,3,4)] <- varia
studentRecord
```
...
length(commitCheck)
num <- c(1:30)
nameofcommitcheck <- str_c("commit",num)
#nameofcommitcheck
names(commitCheck) <- nameofcommitcheck
#names(commitCheck)
commitCheck
```
...
```{r ans26}
logy1 <- hour(allCommitDatesTW)>=5 & hour(allCommitDatesTW)<6
logy2 <- hour(allCommitDatesTW)>=4 & minute(allCommitDatesTW)>=30 &hour(allCommitDatesTW)<5
logy3 <- hour(allCommitDatesTW)>=6 & minute(allCommitDatesTW)<=30 &hour(allCommitDatesTW)<7

logicWithinTimeFrame<- logy1|logy2|logy3
...
```{r ans34}
test2 <- transcriptData[transcriptData$學系 == "經濟學系", ]
transcriptPartialDataEcon <- test2[c(1,7,8)]
